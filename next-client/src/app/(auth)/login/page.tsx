...
// ─── Social login button (Google / LinkedIn) ──────────────────────────────────
      ...
      <div className="space-y-1.5">
        <div className="flex items-center justify-between">
          ...
          <Link
            ...
            className="text-xs font-medium text-[#1D4ED8] hover:underline underline-offset-2"
          >
            Forgot password?
          </Link>
        </div>
...
// ─── Candidate login (full featured) ─────────────────────────────────────────

function CandidateLogin({ onLogin }: { onLogin: (email: string) => void }) {
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <SocialButton
          icon={<LinkedInIcon />}
          label="LinkedIn"
          onClick={() => onLogin("linkedin-user@linkedin.com")}
        />
      </div>
...
// ─── Simple login (other roles) ───────────────────────────────────────────────

function SimpleLogin({ onLogin }: { onLogin: (email: string) => void }) {
  const emailRef = useRef<HTMLInputElement>(null);
  const [showPw, setShowPw] = useState(false);

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onLogin(emailRef.current?.value?.trim() ?? "");
  };
...
export default function LoginPage() {
  ...
  const { title, subtitle } = useMemo(() => {
    switch (role) {
      ...
      case "administer":
      case "admin":
        return { title: "Admin Console", subtitle: "Manage users, roles, and system settings" };
      default:
  ...
  const handleLogin = useCallback(
    (email: string) => {
      const currentRole = params.get("role") ?? "candidate";
  ...
  const isCandidate = role === "candidate" || !params.get("role");
  ...
  return (
    <div className="rounded-2xl border border-[#E5E7EB] bg-white p-8 shadow-[0_4px_24px_-4px_rgba(12,24,55,0.12)]">
      ...
      {isCandidate ? (
        <CandidateLogin onLogin={handleLogin} />
      ) : (
