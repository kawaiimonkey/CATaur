    ...
    <div className="p-8 space-y-8">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <Button variant="outline" className="gap-2 bg-white text-sm h-9 border-slate-200 text-slate-600 cursor-pointer hover:text-slate-800">
      ...
      <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
        {KPI_DATA.map((kpi) => (
          <div key={kpi.title} className="group relative overflow-hidden rounded-2xl border border-slate-200/80 bg-white p-6 transition-all duration-300 hover:shadow-lg hover:shadow-slate-200/50 cursor-pointer hover:border-slate-300/80">
        ...
        <div className="xl:col-span-2 rounded-2xl border border-slate-200/80 bg-white">
          <div className="flex items-center justify-between border-b border-slate-100 px-6 py-5">
            ...
            <div className="flex items-center gap-4">
              ...
              <button className="text-xs font-medium text-slate-400 cursor-pointer hover:text-slate-600 transition-colors">View Logs</button>
            </div>
          </div>
          <div className="p-6">
          ...
          <div className="grid gap-4">
            {QUICK_LINKS.slice(0, 3).map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="group flex items-center gap-4 rounded-xl border border-slate-200/80 bg-white p-4 transition-all duration-300 hover:shadow-md cursor-pointer hover:border-slate-300"
              >
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-sm font-semibold text-slate-900 group-cursor-pointer hover:text-violet-600 transition-colors">{link.label}</h3>
                  <p className="text-xs text-slate-500 mt-0.5 truncate">{link.desc}</p>
                </div>
                <ArrowRight className="h-4 w-4 text-slate-300 group-cursor-pointer hover:text-violet-500 transition-colors" />
              </Link>
            ))}
          </div>
        </div>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        ...
        <div className="rounded-2xl border border-slate-200/80 bg-white">
          <div className="flex items-center justify-between border-b border-slate-100 px-6 py-5">
            ...
            <Link href="/administer/activity" className="text-xs font-medium text-blue-600 cursor-pointer hover:text-blue-700 transition-colors">View All</Link>
          </div>
          <div className="divide-y divide-slate-50">
            {RECENT_ACTIVITIES.map((item, idx) => {
              ...
              return (
                <div key={idx} className="flex gap-3 px-6 py-3.5 transition-colors cursor-pointer hover:bg-slate-50/50">
        ...
        <div className="rounded-2xl border border-slate-200/80 bg-white">
          <div className="flex items-center justify-between border-b border-slate-100 px-6 py-5">
            ...
            <Link href="/administer/users" className="flex items-center gap-1 text-xs font-medium text-blue-600 cursor-pointer hover:text-blue-700 transition-colors">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-50">
                {SAMPLE_USERS.map((u) => (
                  <tr key={u.email} className="group transition-colors cursor-pointer hover:bg-slate-50/50">
                    ...
                    <td className="px-6 py-3.5 text-right">
                      <Button variant="ghost" size="sm" className="h-7 text-xs text-slate-400 cursor-pointer hover:text-slate-600">
                        Edit
                      </Button>
                    </td>
                  </tr>
